#' Scaled Vecchia Gaussian Process
#'
...
#'
#' @param X nxd matrix of input coordinates
#' @param y vector of length n
#' @param ms vector of conditioning-set sizes
#' @param trend options are 'pre' (subtract sample mean as a preprocessing step), zero' (no trend), 'intercept', 'linear' (incl intercept)
#' @param X_trend nxp trend matrix (use if more complicated trend is desired)
...
#' @param select un-select input variables if estimated range parameter isabove select (assuming standardized [0,1] inputs)
...
#'
#' @return Object containing fit information, including for use in predict.svecgp()
#' @examples
...
svecgp <- function(X, y,ms=c(30),trend='pre',X_trend,nu=3.5,nug=0,scale='parms',
  ...
  if(missing(X_trend)) {
    if(trend=='zero'){
      X_trend=as.matrix(sample(c(-1,1),n,replace=TRUE))
    } else if(trend=='intercept'){
      X_trend=as.matrix(rep(1,n))
    } else if(trend=='linear'){
      X_trend=cbind(rep(1,n),inputs)
    } else if(trend=='pre'){
      X_trend=as.matrix(sample(c(-1,1),n,replace=TRUE))
      beta=mean(y)
      y=y-beta
  ...
  if(missing(var.ini)) {
    cur.var=summary(stats::lm(y~X_trend-1))$sigma^2
  ...
  if(n.est<n){
    ...
    y.full=y; inputs.full=inputs; X.full=X_trend
    y=y[ind.est]; inputs=inputs[ind.est,,drop=FALSE]; X=X_trend[ind.est,,drop=FALSE]
  }
  ...
  if(joint){  # joint predictions
    ...
    for(i in 1:n_pred){
      ...
      cl <- tryCatch(
        t(chol(K)),
        error = function(e) {
          # Try again with a tiny nugget
          nugget <- 1e-6
          K_stable <- K + diag(nugget, nrow(K))
          tryCatch(
            t(chol(K_stable)),
            error = function(e2) {
              stop("Cholesky failed even after adding nugget: ", conditionMessage(e2))
            }
          )
        }
      )
...
fit_vcf=function(fit,m.pred=140,n.test=min(1e3,round(nrow(fit$locs)/5)),
  ...
  # make predictions
  class(fitsearch) <- "svecgp"
  preds=predict(fitsearch, newdata=fit$locs[inds.test,,drop=FALSE],
                           m=m.pred, joint=FALSE, predvar=TRUE, scale=scale, samples=0,
...
#' Plot Method for class svecgp
#'
#' See \code{svecgp} for details.
#'
#' @param x An object returned by the \code{svecgp} function.
#' @param ... additional arguments passed to \code{plot}
#' @details Plot function for mpgp.
#' @examples
#' X <- lhs::maximinLHS(100, 2)
...
#' plot(fit)
#' @export
plot.svecgp <- function(x, ...){
  pred <- predict(x, x$locs, samples=1000)
  yhat <- colMeans(pred)
  plot(x$y, yhat, ...)
  graphics::abline(0, 1, lwd=2, col='orange')
